            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h3 class="text-lg font-semibold">{{ __('Stock Adjustments') }}</h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                {{ __('Manual corrections to stock levels at specific locations.') }}
                            </p>
                        </div>
                                ...
                                @forelse($adjustments as $adjustment)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $adjustment->product->description ?? $adjustment->product->item_code }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                            {{ $adjustment->location->name }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-right">
                                            <span class="{{ $adjustment->quantity_change >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                                            {{ __('No adjustments recorded yet.') }}
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
