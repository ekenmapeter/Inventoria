            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('adjustments.store') }}" class="space-y-6">
                        ...
                        <div>
                            <label for="quantity_change" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                {{ __('Quantity Change') }}
                            </label>
                            <input type="number" id="quantity_change" name="quantity_change"
                                   value="{{ old('quantity_change') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                   required>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                {{ __('Use positive values to increase stock, negative to decrease.') }}
                            </p>
